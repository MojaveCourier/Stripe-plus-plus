...
# 定义源文件路径
SOURCE_FILE="/users/Fengming/Stripe-plus-plus/small_tools/generator_sh.py"
...
for HOST in $HOSTS; do
  ...
  scp "$SOURCE_FILE" "$HOST:/users/Fengming/Stripe-plus-plus/small_tools/"
  if [ $? -eq 0 ]; then
...
REMOTE_COMMAND="cd /users/Fengming/Stripe-plus-plus/small_tools/ && python generator_sh.py"
PARALLEL=50
USER="root"
