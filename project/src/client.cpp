...
namespace ECProject
{
  ...
  */
  ...
  {
    std::string coding_mode = m_sys_config->CodingMode;
    if(coding_mode == "ElasticMode"){
      grpc::ClientContext context;
      coordinator_proto::RequestProxyIPPort request;
      coordinator_proto::ReplyProxyIPsPorts reply;
    ...
    else if(coding_mode == "ReplicationMode"){
      grpc::ClientContext context;
      coordinator_proto::RequestProxyIPPort request;
      coordinator_proto::ReplyProxyIPsPorts reply;
      request.set_key(object_id);
      ...
      std::cout << "Unsupported Coding Mode!" << std::endl;
      return false;
    }
  }
  ...
  std::shared_ptr<char []> Client::get_object(std::string key, int block_cnt)
  {
    grpc::ClientContext context;
    coordinator_proto::KeyAndClientIP request;
    request.set_key(key);
    ...
    std::thread notify_thread([&context, &request, &reply, this, &is_get_blocks]() {
      grpc::Status status;
      status = m_coordinator_ptr->getObject(&context, request, &reply);
      if (status.ok())
      {
        is_get_blocks = true;
      }
